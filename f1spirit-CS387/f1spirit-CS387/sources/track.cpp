...
void CTrack::draw(SDL_Rect *vp, float x, float y, float zoom, List<CPlacedGLTile> *extras, GLTile **road_textures, GLTile **lroad_textures, GLTile **rroad_textures, GLTile *road_line)
{
	...
	{
		glPushMatrix();
		glTranslatef(pit_x[0]+start_x, pit_y[0]+start_y,0);
		glRotatef(pit_angle, 0,0,1);
		float px[] = {0,pit_x[1]-pit_x[0]};
		float py[] = {0,pit_y[1]-pit_y[0]};
		C2DCMC *tmp_cmc = new C2DCMC(px,py,2);
		tmp_cmc->draw(1,1,1,1);
		delete tmp_cmc;
		glPopMatrix();		
	}
	*/
