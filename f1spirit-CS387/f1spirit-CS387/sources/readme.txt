...
- the ZOOM effect (the default is SPEED STEPPED)
		...
		- FIXED:
the zoom is always 1x

- SPEED LINEAR:
the zoom decreases linearly as the speed of your
car increases

- SPEED STEPPED:
there are 3 different zooms,
...
- OVERALL TOP 25:
is a ranking sorted by the top 25 players with the
highest accumulated points. In the case of a tie, the
	...
	- BEST TRACK TIMES:
keeps a record of which player has finished each track
in the minimum time.
...
- BODY:
3 different bodies, stronger but heavyer bodies or weaker but
lighter bodies.

- ENGINE:
6 different motors with different power and fuel consumption
...
- SUSPENSION:
	3 different settings that determine the sharpness of the
	turns you can do.

- GEAR:
	3 different settings for the gear shifting box.
			...
			turning (as in real cars), moreover, your car only consumes fuel when you
				...
				To win an F1 race, try to memorize each curve, play several times to each
					...
					Future improvements (planned but not implemented):
								...
								- personal results:
								best lap, position, and points per track
								...
								- improved online highscores:
								accumulated time, top 3 best times per track
								...
								- Shown the point requirements for each unavailable track
									...
									- Configuration file, now the game remeber if the user executed it in
										...
										Graphics:
										RamonMSX, Miikka "MP83" Poikela, Valerian, Olivier "Picili",
										Matriax
										...
										- Joram "Daedalus" van Hartingsveldt:
										png format reduction tools
										...
										- Ootini:
										scaned original manual of F1 - Spirit
