...
bool F1SpiritGame::cycle(KEYBOARDSTATE *k)
{
	...
	*/
	...
	while (l.Iterate(v)) {
		...
		{
			int turn = 0, gear = 0;
            bool accelerate = false, brake = false;
            
//            v->PlayerAICS480_keyboard(&accelerate,&brake,&turn,&gear,k, cars, car_grid, car_grid_sx, car_grid_sy);            
            v->PlayerAICS480(&accelerate,&brake,&turn,&gear,k, &race_positions, car_grid, car_grid_sx, car_grid_sy);            

			v->car->cycle(accelerate,
			              brake,
			              gear,
			              turn,
